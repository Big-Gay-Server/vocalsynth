...
function stripAccents($str) {
    // This replaces accented characters with their plain counterparts
...
// idk i'll title this later
function getCovers($charaFolder = null) {
    $all_covers = [];
    $folders = glob(__DIR__ . '/../covers/*/', GLOB_ONLYDIR);

    // 1. First, gather all covers
    ...
    // 2. Second, build the $vb_map (needed for filtering)
    $vb_map = [];
    ...
    // 3. Third, filter the covers using the map
    if ($charaFolder) {
        $all_covers = array_filter($all_covers, function($cover) use ($charaFolder, $vb_map) {
            $vbs_in_cover = $cover['voicebank'] ?? '';
            $vb_list = is_array($vbs_in_cover) ? $vbs_in_cover : [$vbs_in_cover];

            foreach ($vb_list as $vb_name) {
                // Check if this VB name exists in our map
                $path = $vb_map[strtolower(trim((string)$vb_name))] ?? '';
                
                $cleanPath = str_replace('é', 'e', strtolower($path));
                $cleanFolder = str_replace('é', 'e', strtolower($charaFolder));
                
                if ($path !== '' && str_contains($cleanPath, $cleanFolder)) {
                    return true;
                }
            }
            return false;
...
// cover grid
function renderCoverGrid($covers, $vb_map = []) {
